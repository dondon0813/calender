...
## 業務規模與決策原則（2026-08-12 使用者親口確認，往後所有建議都要照這個算）

這不是小型副業試水溫的專案，是正式事業：使用者社群擁有 10 萬粉絲，前台流量註定會很大；單日轉單金額可達數十萬新台幣；使用者年收入約新台幣一千萬（換算時薪約 NT$5,000，每分鐘約 NT$83）。**未來明確會走向開放註冊、給其他公司使用的多租戶產品**——現在的 Google Sheets＋Apps Script 架構是過渡期方案，不是要拿去長大的地基。

這對「給建議」的實際影響：
- **不要預設省錢／免費優先。** 判斷準則永遠是「這筆錢／這個方案，相對使用者的時間成本與營收風險，划不划算」，不是「有沒有更便宜的做法」。多數情況下小額付費（GAS 配額、Workspace、雲端主機…）遠比使用者自己花時間排查或冒營收風險划算，遇到這種情況直接建議付費方案，不要因為「本來想省錢」而繞去 DIY。
- **診斷/排錯要考慮使用者時間成本。** 每多一輪來回確認（截圖、等待、重試）都是真金白銀的時間成本，能一次講清楚就不要分批問；能用低成本方案先擋著風險（例如先買保險性質的升級）、事後再確認根因，優於堅持「先百分之百確認因果才能花錢」。
- **重大架構決策仍要讓使用者拍板**（例如是否啟動正式後端重寫、要不要開始找工程團隊），但建議時要把「拖延的代價」講清楚：現有系統累積的資料/功能越多，未來轉移到正式多租戶架構的工程量與陣痛就越大，不是「以後再說」沒有成本。
...
3. **實作者不得自我驗證**：改動完成後派 fresh-context subagent 驗收（規則在 20 號檔）。公開端點（scope=public / brands / brandThumbs）只准輸出品牌名稱、介紹、去背小圖、蝦皮連結（2026-07-27 起；導購網址本來就是要給客人點的）——多吐一個欄位就是資安事故。
