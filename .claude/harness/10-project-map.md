...
| 檔案 | 行數 | 用途 | 分級 |
|---|---|---|---|
| admin.js | 3869 | 後台前端邏輯主檔（見 §5 區塊索引；已拆出 7 個模組，見「已拆出的模組」） | 🔴 |
| admin.html | 5315 | 員工後台頁（登入、行事曆編輯、任務、圖片庫、資料庫管理…） | 🔴 |
| Code.gs | 4057 | GAS 後端全部邏輯（見 §4 端點目錄） | 🔴 |
...
| brandVendor.js | 710 | 廠商/品牌資料庫管理＋檢視彈窗＋排行事曆品牌比對＋開團貼文文案產生（evPostGenBtn／postTemplate，2026-08-03）（**同樣必須排在 admin.js 之前**） | 🟢 |
